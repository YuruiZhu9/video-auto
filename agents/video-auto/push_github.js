...
async function getFileSha(path) {
  try {
    const res = await httpReq('GET', `/repos/${REPO}/contents/${path}?ref=${BRANCH}`);
...
async function uploadFile(repoPath, localPath, message) {
  ...
  if (res.status === 201 || res.status === 200) {
    console.log(`  Success: ${res.data.content?.html_url || 'OK'}`);
    return true;
  } else {
    console.log(`  Error:`, JSON.stringify(res.data).substring(0, 300));
    return false;
  }
}

async function main() {
  console.log('=== GitHub Push: YuruiZhu9/video-auto ===\n');

  // Check if repo exists
  const repoRes = await httpReq('GET', `/repos/${REPO}`);
  console.log(`Repo check: ${repoRes.status}`);
  if (repoRes.status !== 200) {
    console.log('Repo not accessible:', repoRes.data);
    return;
  }
  ...
  // 1. slides/output.html
  if (await uploadFile('slides/output.html', '/workspace/agents/video-auto/slides/output.html', 'Update slides/output.html')) success++; else fail++;

  // 2. slides/slides_grid.png
  if (await uploadFile('slides/slides_grid.png', '/workspace/agents/video-auto/slides/slides_grid.png', 'Add slides grid preview')) success++; else fail++;

  // 3. content/script.md
  if (await uploadFile('content/script.md', '/workspace/agents/video-auto/content/script.md', 'Update content/script.md')) success++; else fail++;

  // 4. video/slides_grid.png
  if (await uploadFile('video/slides_grid.png', '/workspace/agents/video-auto/video/slides_grid.png', 'Add video slides_grid.png')) success++; else fail++;

  // 5. OPTIMIZATION.md
  const optContent = `# Video-Auto Pipeline 优化方案
...
`;
  fs.writeFileSync('/workspace/agents/video-auto/OPTIMIZATION.md', optContent);
  console.log('\nOPTIMIZATION.md written');
